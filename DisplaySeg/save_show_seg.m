function save_show_seg(segments, img, Show, Save, Save_all, img_name, save_path, result_mat_path)
    ...
    if Show
        figure(1), imshow(uint8(imgMarkup)) ; title([img_name,'   ', num2str(Rnum)]) ; % Show boundary
        figure(2), imshow(uint8(MeanImMarkup)) ; title([img_name, num2str(Rnum)]) ; % Show mean color
    end
    if Save
