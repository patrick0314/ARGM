...
fid = fopen(fullfile('results2', 'myNsegs.txt'), 'r') ; % read the file .\results\BSDS300\myNsegs.txt
...
result_mat_path = 'results2\Label_mat\' ;
...
fid_out = fopen(fullfile('results2', 'Evaluations.txt'), 'w') ;

%%% record the auto-seg number
fid_outout = fopen(fullfile('results2', 'Nseg.txt'), 'w') ;
...
for idx = 1:75
    ...
    fprintf(fid_outout, '%6d:   new:%2d   old:%2d\r\n', BSDS_INFO(1, idx), Nseg, BSDS_INFO(2, idx));
    %fprintf(fid_outout, '%6d:  %2d\r\n', BSDS_INFO(1, idx), Nseg);
